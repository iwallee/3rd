 ...
 * $Id: me.c,v 1.1.1.1 2005/03/16 13:27:22 264 Exp $
 *
...
//这一步的操作有待进一步的验证 
    ...
    if( h->mb.i_subpel_refine >= 2 )
    {
        ...
        omx = bmx;
        omy = bmy;
		while ((mvh!=0 || mvv!=0) && i_iter<8) {
			if(mvv==0)
			{
				COST_MV( omx+ mvh, omy   );
				COST_MV( omx+ mvh/2, omy+2 );
				COST_MV( omx+ mvh/2, omy-2 );
			}
			else
			{
				COST_MV( omx+ mvh*2, omy   );
				COST_MV( omx-1, omy+mvv );
				COST_MV( omx+1, omy+mvv );
			}
			mvh=bmx-omx;
			mvv=bmy-omy;
