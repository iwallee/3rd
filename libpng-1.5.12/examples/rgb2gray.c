 ...
 * rgb2gray.c
 *
 * converts any APNG to greyscale APNG.
 *
...
void LoadAPNG(char * szImage, image_info ** img, unsigned int * w, unsigned int * h, unsigned int * d, unsigned int * t, png_uint_32 * frames, png_uint_32 * plays, unsigned int * hidden)
{
  FILE * f1;
    ...
    if (fread(sig, 1, 8, f1) == 8 && png_sig_cmp(sig, 0, 8) == 0)
    {
      ...
      if (png_ptr != NULL && info_ptr != NULL && setjmp(png_jmpbuf(png_ptr)) == 0)
      {
        ...
        *hidden = png_get_first_frame_is_hidden(png_ptr, info_ptr);
        if (!png_get_acTL(png_ptr, info_ptr, frames, plays))
#endif
        {
          *frames = 1;
          *plays  = 0;
        ...
        *img = (image_info *)malloc(*frames*sizeof(image_info));
        if (*img != NULL)
        {
          ...
          for (i=0; i<*frames; i++)
          {
            ...
            if (!png_get_next_frame_fcTL(png_ptr, info_ptr, &pimg->w0, &pimg->h0, &pimg->x0, &pimg->y0, &pimg->delay_num, &pimg->delay_den, &pimg->dop, &pimg->bop))
#endif
            {
              pimg->x0 = 0;
              pimg->y0 = 0;
...
void SaveAPNG(char * szImage, image_info * img, unsigned int w, unsigned int h, unsigned int d, unsigned int t, png_uint_32 frames, png_uint_32 plays, unsigned int hidden)
{
  FILE * f1;
...
int main(int argc, char** argv)
{
  ...
  if (argc <= 1)
    printf("Usage : rgb2gray input.png\n");
  else
  {
    LoadAPNG(argv[1], &img, &w, &h, &d, &t, &frames, &plays, &hidden);
    SaveAPNG("test_rgb2gray.png", img, w, h, d, t, frames, plays, hidden);
