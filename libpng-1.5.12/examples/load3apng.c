...
void SaveTGA(unsigned char ** rows, unsigned int w, unsigned int h, unsigned int channels, unsigned int frame)
{
  char szOut[512];
  FILE * f2;
  if (channels == 4)
  {
    ...
    sprintf(szOut, "test_load3_%03d.tga", frame);
...
{
  if (argc <= 1)
    printf("Usage : load3apng input.png\n");
  else
    LoadPNG(argv[1]);
