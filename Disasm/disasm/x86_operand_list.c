...
x86_op_t * x86_operand_new( x86_insn_t *insn ) {
	...
	if (! insn || insn->allocops >= 
		sizeof(insn->freeops)/sizeof(insn->freeops[0])) {	
		return(NULL);
	}
	op = &insn->freeops[insn->allocops++];
...
*/
	insn->operands = NULL;
	insn->allocops = 0;
	memset(insn->freeops, 0, sizeof(insn->freeops));
...
int x86_operand_foreach( x86_insn_t *insn, x86_operand_fn func, void *arg, 
	...
	type = type & 0xF0; /* mask out explicit/implicit operands */
